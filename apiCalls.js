...
function insertUser(userID, username, email, userType) {
...
{
    connection.query('SELECT  FROM Reply WHERE id = ?', [replyID], function(error, results, fields) {
        if (error) {
            callback(error);
        } else {
...
function getThreadReplies(threadID, callback) {
    ...
    WITH RECURSIVE thread_replies AS (
      ...
      SELECT r.*
      FROM reply r
      JOIN thread_replies tr ON r.ReplyToID = tr.id
    )
    SELECT *
    FROM thread_replies
    ORDER BY timestamp ASC;
  `;
    connection.query(sql, [threadID], (error, results) => {
        if (error) {
            callback(error);
        } else {
            callback(null, results);
...
function getThread(threadID, callback) {
    connection.query('SELECT * FROM thread WHERE id = ?', [threadID], function(error, results, fields) {
        if (error) {
            callback(error);
        } else {
            callback(null, results[0]);
...
console.log("testing");
insertUser("0000001232", "testKid1", "testKid1@gmail.com", "student")
let callback  =  null;
changeUserType('0000000001', 'illegal', (error, result) => {
  if (error) {
    console.error(error);
  } else {
    callback = result;
